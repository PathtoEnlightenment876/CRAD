...
<div class="container">
    <div class="card">
        <h2 class="text-center mb-4">Department Assignments</h2>
        
        <ul class="nav nav-pills justify-content-center mb-3" id="pills-tab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="pills-it-tab" data-bs-toggle="pill" data-bs-target="#pills-it" type="button" role="tab" aria-controls="pills-it" aria-selected="true">IT</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="pills-educ-tab" data-bs-toggle="pill" data-bs-target="#pills-educ" type="button" role="tab" aria-controls="pills-educ" aria-selected="false">EDUC</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="pills-crim-tab" data-bs-toggle="pill" data-bs-target="#pills-crim" type="button" role="tab" aria-controls="pills-crim" aria-selected="false">CRIM</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="pills-bsba-tab" data-bs-toggle="pill" data-bs-target="#pills-bsba" type="button" role="tab" aria-controls="pills-bsba" aria-selected="false">BSBA</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="pills-bshm-tab" data-bs-toggle="pill" data-bs-target="#pills-bshm" type="button" role="tab" aria-controls="pills-bshm" aria-selected="false">BSHM</button>
            </li>
        </ul>
            ...
            <div class="tab-pane fade show active" id="pills-it" role="tabpanel" aria-labelledby="pills-it-tab">
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tbody id="itAssignmentsBody"></tbody>
                    </table>
                </div>
            </div>
            ...
            <div class="tab-pane fade" id="pills-educ" role="tabpanel" aria-labelledby="pills-educ-tab">
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tbody id="educAssignmentsBody"></tbody>
                    </table>
                </div>
            </div>
            ...
            <div class="tab-pane fade" id="pills-crim" role="tabpanel" aria-labelledby="pills-crim-tab">
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tbody id="crimAssignmentsBody"></tbody>
                    </table>
                </div>
            </div>
            
            <div class="tab-pane fade" id="pills-bsba" role="tabpanel" aria-labelledby="pills-bsba-tab">
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        <thead>
                            <tr>
                        ...
                        <tbody id="bsbaAssignmentsBody"></tbody>
                    </table>
                </div>
            </div>

            <div class="tab-pane fade" id="pills-bshm" role="tabpanel" aria-labelledby="pills-bshm-tab">
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        <thead>
                            <tr>
                        ...
                        <tbody id="bshmAssignmentsBody"></tbody>
                    </table>
                </div>
            </div>
        </div>
...
<script>
    // This is where you would get the data from your assignment forms.
    // For this example, we'll use a placeholder array.
    const assignments = [
        { group: '101', department: 'IT', adviser: 'Dr. Mark Johnson', panel: 'Prof. Maria Davis, Dr. Kevin Brown' },
        { group: '102', department: 'IT', adviser: 'Dr. Sarah Lee', panel: 'Dr. Alan Smith, Prof. Maria Davis' },
        { group: '201', department: 'EDUC', adviser: 'Prof. Jane Williams', panel: 'Dr. Richard Jones, Prof. Linda Evans' },
        { group: '202', department: 'EDUC', adviser: 'Dr. Robert Taylor', panel: 'Dr. Nancy White, Prof. Paul Green' },
        { group: '301', department: 'CRIM', adviser: 'Dr. Alex Carter', panel: 'Prof. Susan King, Dr. Ben Lee' },
        { group: '401', department: 'BSBA', adviser: 'Dr. Chris Evans', panel: 'Dr. Jessica Adams, Prof. Gary Hall' },
        { group: '501', department: 'BSHM', adviser: 'Prof. Emily Wilson', panel: 'Dr. David Kim, Dr. Olivia Chen' },
    ];

    const itAssignmentsBody = document.getElementById('itAssignmentsBody');
    const educAssignmentsBody = document.getElementById('educAssignmentsBody');
    const crimAssignmentsBody = document.getElementById('crimAssignmentsBody');
    const bsbaAssignmentsBody = document.getElementById('bsbaAssignmentsBody');
    const bshmAssignmentsBody = document.getElementById('bshmAssignmentsBody');

    // Function to render the table for a specific department
    const renderTable = (dataToRender, tableBody) => {
        tableBody.innerHTML = ''; // Clear existing rows
        if (dataToRender.length === 0) {
            tableBody.innerHTML = `<tr><td colspan="3" class="text-center text-muted">No assignments found for this department.</td></tr>`;
            return;
        }
        dataToRender.forEach(assignment => {
            const row = document.createElement('tr');

            // Split panel names by comma and wrap each in a div for stacking
            const panelNames = assignment.panel.split(',').map(name => `<div>${name.trim()}</div>`).join('');
            
            row.innerHTML = `
                <td>${assignment.group}</td>
                <td>${assignment.adviser}</td>
                <td class="stacked-names">${panelNames}</td>
            `;
            tableBody.appendChild(row);
        });
    };

    // Filter and render data for each department
    const renderAllDepartments = () => {
        const departments = ['IT', 'EDUC', 'CRIM', 'BSBA', 'BSHM'];
        const tableBodies = [itAssignmentsBody, educAssignmentsBody, crimAssignmentsBody, bsbaAssignmentsBody, bshmAssignmentsBody];

        departments.forEach((dept, index) => {
            const filteredData = assignments.filter(item => item.department === dept);
            renderTable(filteredData, tableBodies[index]);
        });
    };

    // Initial render when the page loads
    document.addEventListener('DOMContentLoaded', renderAllDepartments);
</script>
</html>